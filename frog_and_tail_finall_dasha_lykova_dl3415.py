...
print(roc_means_leiden.head(8))  # should start with '18'
roc_means_leiden.to_csv(os.path.join(OUT_DIR, "roc_gene_score_by_cluster.csv"))
...
# ROC-like feature maps with a shared vmax (fixed display)
import os
import numpy as np
import matplotlib.pyplot as plt
import scipy.sparse as sp
import scanpy as sc
from IPython.display import Image as IPyImage, display  # <-- correct Image
...
# genes to plot (use exact var_names you picked earlier)
genes = ['egfl6.S','frem2.1.L','pltp.S','nid2.L','igfbp2.S']
panel_titles = [g.upper() for g in genes]

# compute a common upper cap (99th percentile) across all genes
X = adata.raw[:, genes].X if adata.raw is not None else adata[:, genes].X
Xd = X.toarray() if sp.issparse(X) else X
vmax = float(np.nanquantile(Xd, 0.99))  # robust to NaNs

# plot UMAP feature panels (no auto-show)
sc.pl.umap(
    adata,
    color=genes,
    use_raw=True,
...
cl = str(globals().get("roc_cluster_leiden", globals().get("roc_cluster", "18")))
fig.suptitle(f"ROC-like genes (shared upper cap={vmax:.2f})", fontsize=18)
...
print("note: this feature panel reflects expression only; it does not depend on the clustering method.")
...
# Cluster-agnostic ROC stability via gene-set score (top-N cells), aligned by obs_names
...
# --- get baseline top-N size from the baseline ROC cluster ---
N_base = int((adata.obs["leiden"].astype(str) == str(roc_cluster_leiden)).sum())
print("Baseline ROC size (N):", N_base)

# --- helper: compute ROC score on an AnnData, return top-N boolean mask indexed by obs_names ---
def topN_mask_by_score(ad, gene_list_varnames, N):
    # compute score if missing
    if "roc_score" not in ad.obs.columns:
        sc.tl.score_genes(ad, gene_list=gene_list_varnames, score_name="roc_score", use_raw=(ad.raw is not None))
    s = ad.obs["roc_score"].copy()
    # pick top-N by score (ties broken by rank)
    ranks = s.rank(method="first", ascending=False)
    mask = ranks <= N
    return pd.Series(mask.values, index=ad.obs_names)

# prepare ROC gene list in var_names space (you already did this elsewhere; re-derive if needed)
# if you already have 'roc_gene_list_var' (mapped varnames), reuse it; otherwise fall back to symbol_clean mapping.
if "roc_gene_list_var" not in globals() or not roc_gene_list_var:
    # naive fallback: try to map S3 symbols to current var_names via symbol_clean
    def _clean_sym(x):
        s = str(x).upper().split(".")[0].split("_")[0]
        return s
    var_df = adata.raw.var if adata.raw is not None else adata.var
    if "symbol_clean" not in var_df.columns:
        var_df["symbol_clean"] = [ _clean_sym(g) for g in (adata.raw.var_names if adata.raw is not None else adata.var_names) ]
    sym2vn = {}
    names  = (adata.raw.var_names if adata.raw is not None else adata.var_names)
    for sym in set([_clean_sym(g) for g in roc_ref_set]):
        hits = np.where(var_df["symbol_clean"].values == sym)[0]
        if hits.size:
            sym2vn[sym] = names[hits[0]]
    roc_gene_list_var = list(sym2vn.values())

print("ROC gene list (varnames) length:", len(roc_gene_list_var))

#compute baseline top-N mask by ROC score
mask_base = topN_mask_by_score(adata, roc_gene_list_var, N_base)

# baseline UMAP coords + scale for centroid shift
coords = adata.obsm["X_umap"]
base_c = coords[mask_base.values, :].mean(axis=0)
umap_span = np.ptp(coords, axis=0)
umap_norm = float(np.linalg.norm(umap_span) + 1e-12)

def jaccard_series(a: pd.Series, b: pd.Series):
    # align by obs_names
    aa = a.reindex(b.index).fillna(False)
    bb = b.fillna(False)
    inter = (aa & bb).sum()
    union = (aa | bb).sum()
    return inter / union if union else np.nan

def centroid_shift_rel(mask_variant: pd.Series):
    mv = mask_variant.reindex(adata.obs_names).fillna(False).values
    if mv.any():
        var_c = coords[mv, :].mean(axis=0)
        shift_abs = float(np.linalg.norm(var_c - base_c))
        return shift_abs, float(shift_abs / umap_norm)
    return np.nan, np.nan

rows = []
for label, ad_var in [
    ("MAGIC",   globals().get("ad_magic", None)),
    ("scVI",    globals().get("ad_scvi",  None)),
    ("Harmony", globals().get("ad_hm",    None)),
    ("BBKNN",   globals().get("ad_bb",    None)),
]:
    if ad_var is None:
        rows.append({"Variant": label, "Jaccard": np.nan, "CentroidShift(abs)": np.nan, "CentroidShift(rel)": np.nan})
        continue
    # top-N mask in variant
    mv = topN_mask_by_score(ad_var, roc_gene_list_var, N_base)
    # align sets and compute Jaccard
    jac = jaccard_series(mask_base, mv)
    # centroid shift in baseline UMAP
    sh_abs, sh_rel = centroid_shift_rel(mv)
    rows.append({"Variant": label, "Jaccard": jac, "CentroidShift(abs)": sh_abs, "CentroidShift(rel)": sh_rel})

stab_score = pd.DataFrame(rows)
print("\nROC stability by score (top-N) vs baseline:")
print(stab_score.to_string(index=False, float_format=lambda x: f"{x:.3f}"))

out_csv = os.path.join(OUT_DIR, "roc_stability_by_score_topN.csv")
stab_score.to_csv(out_csv, index=False)
print("\nSaved:", out_csv)
